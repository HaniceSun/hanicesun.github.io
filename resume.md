...
By reanalyzing both whole exome sequencing and RNA-Seq data of over 10,000 patients across 33 cancer types from the TCGA project using a negative binomial-based generalized linear model, I observed very exciting correlations between somatic mutations and the splicing between the adjacent genes in some cohorts, as well as the overexpression of one of the adjacent genes, including the top hit, mutation within CDKN1A, splicing between CDKN1A and RAB44, and the activation of RAB44 in bladder, stomach, and skin cancer. I established the causality through collaboration with other labs by introducing the mutation in multiple cell lines using CRISPR/Cas9 and then confirming the appearance of the splicing between CDKN1A and RAB44 using RT-PCR and Sanger sequencing and activation of RAB44 using qPCR and RNA-Seq. We further performed functional/phenotyping assays including grafted mouse models  to illustrate the connection from RAB44 activation to MDM2 upregulation and TP53 downregulation. I proposed a splicing-mediated readthrough stabilization (SRS) model as one more mechanism for oncogene activation to explain this type of atypical transcript level fusion events instead of the common gene fusions on the genomic level.
