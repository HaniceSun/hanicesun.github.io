...
By reanalyzing both whole exome sequencing and RNA-Seq data of over 10,000 patients across 33 cancer types from the TCGA project using a generalized linear model, I observed a very interesting correlation between somatic mutations, splicing between the adjacent genes, and the overexpression of the downstream gene in a cohort subset, including the top hit, mutation within CDKN1A, splicing between CDKN1A and RAB44, and the activation of RAB44 in bladder, stomach, and skin cancer. I established the causality through collaboration with other labs by introducing the mutation in cell lines using CRISPR/Cas9. We further performed functional/phenotyping assays to illustrate the connection from RAB44 activation to MDM2 upregulation and TP53 downregulation. I proposed a splicing-mediated readthrough stabilization (SRS) model as one more mechanism for oncogene activation to explain this type of atypical transcript level fusion events instead of the common gene fusions on the genomic level.
